...
let seats = Number(localStorage.getItem("murreeSeats")) || 14;
...
const hotels = {
  ...
  metropole: {
    name: "Metropole Hotel Murree",
    adjustment: 4000,
    label: "Standard package"
  },
  move: {
    name: "Move-N-Pick Murree",
    adjustment: 3000,
    label: "Family package"
  },
  budget: {
    name: "Budget Family Hotel Murree",
    adjustment: 1500,
...
function updateMapAndTopLocation(city) {
  if (!city) {
    routeText.textContent = "Select your starting city to show route to Murree.";
    routeMap.src = "https://www.google.com/maps?q=Murree%20Patriata%20Pakistan&output=embed";
    return;
  }

  routeText.textContent = `${city} → Murree & Patriata`;
  routeMap.src = `https://www.google.com/maps?q=${encodeURIComponent(city + " to Murree Patriata Pakistan")}&output=embed`;
  ...
  function updatePriceAndRoute() {
    ...
    calculatedRoute.textContent = `${city} to Murree & Patriata`;
  ...
  function findPreviousBooking(phone) {
    ...
    return bookings
  .filter(b => b.phone === phone && b.tour === "Murree & Patriata Tour")
  .slice(-1)[0];
  }
  ...
  bookingForm.addEventListener("submit", function (e) {
    ...
    formData.append("to_destination", "Murree & Patriata");
    formData.append("route_text", city + " to Murree & Patriata");
      ...
      .then(data => {
        if (data.success) {
          ...
          confirmationMsg.innerHTML =
            ...
            "Route: <strong>" + city + " to Murree & Patriata</strong><br>" +
  ...
  document.querySelectorAll(".clickable-hotel").forEach(card => {
    card.addEventListener("click", function () {
      ...
      if (hotelLocation && routeMap && routeText) {
