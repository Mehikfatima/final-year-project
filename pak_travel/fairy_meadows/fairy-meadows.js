document.addEventListener("DOMContentLoaded", function () {
  ...
  let seats = Number(localStorage.getItem("fairyMeadowsSeats")) || 18;
  ...
  const priceByCity = {
    ...
    bahawalpur: 65000,
    deraghazikhan: 68000,
    "dera ghazi khan": 68000,
    rahimyarkhan: 70000,
    "rahim yar khan": 70000,
    hyderabad: 76000,
    abbottabad: 45000,
    mansehra: 44000,
    swat: 52000,
  ...
  const hotels = {
    premiumCamp: { name: "Premium Fairy Meadows Camp", adjustment: 12000, label: "Adventure luxury camp" },
    standardCamp: { name: "Standard Camping Package", adjustment: 8000, label: "Comfortable camping" },
    budgetCamp: { name: "Budget Camping Package", adjustment: 5000, label: "Economy camping" },
    cottage: { name: "Fairy Meadows Cottage Stay", adjustment: 10000, label: "Family cottage stay" },
    basic: { name: "Basic Tent Package", adjustment: 3000, label: "Low-budget tent" }
  };
  ...
  function getCityPrice(city) {
    ...
    return priceByCity[normalized] || priceByCity[normalized.replace(/\s+/g, "")] || 0;
  }

  function getTravelerCharge(perPerson, type, age) {
    const travelerAge = Number(age) || 0;
    if (type === "Child") {
      if (travelerAge > 0 && travelerAge <= 4) return { amount: 0, label: "Child 0-4 years: Free" };
      if (travelerAge >= 5 && travelerAge <= 8) return { amount: perPerson * 0.5, label: "Child 5-8 years: 50% charges" };
    }
  ...
  function scrollToMap() {
    document.querySelector(".tour-map-card")?.scrollIntoView({ behavior: "smooth", block: "start" });
  }

  function setActiveCard(card) {
    document.querySelectorAll(".clickable-hotel, .place-card").forEach(item => item.classList.remove("active-map-card"));
    if (card) card.classList.add("active-map-card");
  }

  function updateMapAndTopLocation(city) {
    if (!city) {
      updateMap("Fairy Meadows Pakistan", "Select your starting city to show route to Fairy Meadows.");
      return;
    }
    updateMap(`${city} to Fairy Meadows Pakistan`, `${city} → Fairy Meadows`);
    let matched = false;
  ...
  function updatePriceAndRoute() {
    ...
    if (!city) {
      ...
      if (roomCountText) roomCountText.textContent = `${rooms} camp/room${rooms > 1 ? "s" : ""}`;
    ...
    calculatedRoute.textContent = `${city} to Fairy Meadows Adventure & Camping Tour`;
    selectedHotelText.innerHTML = hotel ? `${hotel.name} <span class="hotel-price-badge">${hotel.label}</span>` : "Please select hotel";
    if (roomCountText) roomCountText.textContent = `${rooms} camp/room${rooms > 1 ? "s" : ""}`;
    if (perPerson > 0) {
      ...
      calculatedPrice.textContent = `${money(total)} (${guests} traveler${guests > 1 ? "s" : ""}, ${rooms} camp/room${rooms > 1 ? "s" : ""})`;
    } else {
    ...
    hotelPriceText.textContent = hotel ? `${money(hotelTotal)} (${money(roomPrice)} x ${rooms} camp/room${rooms > 1 ? "s" : ""})` : "Rs. 0";
    ...
    for (let i = 1; i <= count; i++) {
      ...
      card.innerHTML = `
        ...
        <div class="traveler-card-grid">
          <select class="traveler-type" required><option value="Adult">Adult</option><option value="Child">Child</option></select>
          ...
          <select class="traveler-gender" required><option value="">Gender</option><option value="Male">Male</option><option value="Female">Female</option><option value="Other">Other</option></select>
      ...
      typeSelect.addEventListener("change", function () {
        if (this.value === "Child") {
          card.classList.add("child"); cnicInput.required = false; cnicInput.value = "";
        } else {
          card.classList.remove("child"); cnicInput.required = true;
        }
        updatePriceAndRoute();
      });
  ...
  function findPreviousBooking(phone) {
    ...
    return bookings.filter(b => b.phone === phone && b.tour === "Fairy Meadows Adventure & Camping Tour").slice(-1)[0];
  }
  ...
  function showPreviousOrForm() {
    ...
    if (previous) {
      previousBookingBox.innerHTML = `
        <p><strong>Name:</strong> ${previous.name}</p><p><strong>Phone:</strong> ${previous.phone}</p>
        <p><strong>From:</strong> ${previous.from}</p><p><strong>To:</strong> ${previous.to}</p>
        <p><strong>Stay Package:</strong> ${previous.hotelName || "Not selected"}</p>
        <p><strong>Total Travelers:</strong> ${previous.guests}</p>
        <p><strong>Camps / Rooms:</strong> ${previous.hotelRoomCount || "Not saved"}</p>
        <p><strong>Total Payment:</strong> ${money(previous.totalPayment)}</p>
        <p><strong>Payment Type:</strong> ${previous.paymentType}</p><p><strong>Date:</strong> ${previous.date}</p>`;
  ...
  startBookingBtn.addEventListener("click", function () { hideAll(); authPanel.classList.remove("hidden"); authPanel.scrollIntoView({ behavior: "smooth" }); });

  authTabs.forEach(btn => btn.addEventListener("click", function () {
    authTabs.forEach(b => b.classList.remove("active")); btn.classList.add("active");
    authMode = btn.dataset.auth; authSubmitBtn.textContent = authMode === "login" ? "Login" : "Register";
  ...
  //     if (!user) { user = { name, phone, password }; users.push(user); localStorage.setItem("tourUsers", JSON.stringify(users)); }
  //     currentUser = user; alert("Registration successful.");
  //   } else {
  //     const user = users.find(u => u.phone === phone && u.password === password);
  //     if (!user) { alert("User not found. Please register first or check phone/password."); return; }
  //     currentUser = user;
  //   }
  //   showPreviousOrForm();
  // });
  ...
  newBookingBtn.addEventListener("click", function () { hideAll(); document.getElementById("customerName").value = currentUser.name; document.getElementById("customerPhone").value = currentUser.phone; bookingFormPanel.classList.remove("hidden"); });

  startLocation.addEventListener("change", function () {
    const city = startLocation.options[startLocation.selectedIndex].value;
    if (city === "custom") {
      customLocation.classList.remove("hide"); priceText.textContent = "Custom price will be confirmed";
      routeText.textContent = "Enter your city to show route to Fairy Meadows.";
      routeMap.src = "https://www.google.com/maps?q=Fairy%20Meadows%20Pakistan&output=embed"; return;
    }
    customLocation.classList.add("hide"); customLocation.value = ""; startCity.value = city; updatePriceAndRoute();
  });

  customLocation.addEventListener("input", function () { startCity.value = customLocation.value.trim(); updatePriceAndRoute(); });
  ...
  guestCount.addEventListener("input", function () { renderTravelerFields(); updatePriceAndRoute(); });

  document.querySelectorAll('input[name="paymentType"]').forEach(radio => radio.addEventListener("change", function () { accountBox.classList.toggle("hidden", this.value === "No Advance"); }));

  document.querySelectorAll(".clickable-hotel").forEach(card => {
    card.addEventListener("click", function () {
      const query = this.dataset.hotel;
      const title = this.querySelector("h3")?.textContent || "Selected stay package";
      setActiveCard(this);
      updateMap(query, `${title} location on map`);
      scrollToMap();
    });
  });
  ...
  document.querySelectorAll(".place-card").forEach(card => {
    card.addEventListener("click", function () {
      const query = this.dataset.place;
      const title = this.querySelector("strong")?.textContent || "Selected place";
      setActiveCard(this);
      updateMap(query, `${title} location on map`);
      scrollToMap();
    });
  ...
  bookingForm.addEventListener("submit", function (e) {
    ...
    if (!hotel) {
      alert("Please select camping / stay package.");
      return;
    }
    ...
    formData.append("to_destination", "Fairy Meadows");
    formData.append("route_text", city + " to Fairy Meadows");
    formData.append("hotel_package", hotelChoice.value);
    formData.append("hotel_name", hotel.name);

    // Fairy Meadows form me vehicle/pickup fields nahi hain, is liye blank save ho ga.
      ...
      .then(data => {
        if (data.success) {
          ...
          confirmationMsg.innerHTML =
            ...
            "Route: <strong>" + city + " to Fairy Meadows</strong><br>" +
            "Stay: <strong>" + hotel.name + "</strong><br>" +
            "Camps / Rooms: <strong>" + rooms + "</strong><br>" +
  ...
  document.querySelectorAll(".faq-item button").forEach(btn => btn.addEventListener("click", function () { btn.parentElement.classList.toggle("active"); }));
  updatePriceAndRoute();
});
