...
let seats = Number(localStorage.getItem("neelum valleyGroupTourSeats")) || 18;
...
const hotels = {
  premiumRiver: {
    name: "Premium River View Hotel",
    adjustment: 12000,
    label: "Luxury river-view room"
  },
  standardHotel: {
    name: "Standard Neelum Hotel",
    adjustment: 8000,
    label: "Comfortable room option"
  },
  budgetHotel: {
    name: "Budget Guest House",
    adjustment: 5000,
    label: "Economy room option"
  },
  familyCottage: {
    name: "Family Cottage Stay",
    adjustment: 10000,
    label: "Private family room/cottage"
  },
  camping: {
    name: "Neelum Camping Package",
    adjustment: 6000,
    label: "Adventure camping stay"
  }
};
...
function updateMapAndTopLocation(city) {
  if (!city) {
    routeText.textContent = "Select your starting city to show route to Neelum Valley.";
    routeMap.src = "https://www.google.com/maps?q=Neelum%20Valley%20Azad%20Kashmir%20Pakistan&output=embed";
    return;
  }

  routeText.textContent = `${city} → Neelum Valley`;
  routeMap.src = `https://www.google.com/maps?q=${encodeURIComponent(city + " to Neelum Valley Azad Kashmir Pakistan")}&output=embed`;
  ...
  function updatePriceAndRoute() {
    ...
    calculatedRoute.textContent = `${city} to Neelum Valley Summer Nature Tour`;
 ...
 function findPreviousBooking(phone) {
  ...
  return bookings
    .filter(b => b.phone === phone && b.tour === "Neelum Valley Summer Nature Tour")
    .slice(-1)[0];
}
// function findPreviousBooking(phone) {
//   const bookings = JSON.parse(localStorage.getItem("adminBookings")) || [];
...
// }
  ...
  startLocation.addEventListener("change", function () {
    ...
    if (city === "custom") {
      customLocation.classList.remove("hide");
      priceText.textContent = "Custom price will be confirmed";
      routeText.textContent = "Enter your city to show route to Neelum Valley.";
      routeMap.src = "https://www.google.com/maps?q=Neelum%20Valley%20Azad%20Kashmir%20Pakistan&output=embed";
      return;
    }
  ...
  bookingForm.addEventListener("submit", function (e) {
    ...
    formData.append("to_destination", "Neelum Valley");
    formData.append("route_text", city + " to Neelum Valley");
      ...
      .then(data => {
        if (data.success) {
          ...
          confirmationMsg.innerHTML =
            ...
            "Route: <strong>" + city + " to Neelum Valley</strong><br>" +
            "Hotel: <strong>" + hotel.name + "</strong><br>" +
            "Rooms: <strong>" + rooms + "</strong><br>" +
            "Child Policy: <strong>0-4 free, 5-8 half, 9+ full</strong><br>" +
...
function getBotReply(question){
  ...
  /* ========================= */
    ...
    q.includes("neelum valley")
  ){

    return `
    🏔 <strong>Neelum Valley</strong>
    <br><br>
    ...
    🛣 Route:
    Neelum Valley → Besham → Chilas → Neelum Valley
    <br><br>
  ...
  /* ========================= */
    ...
    q.includes("neelum valley") ||
    q.includes("deosai") ||
    q.includes("shangrila") ||
    q.includes("katpana") ||
    q.includes("upper kachura")
  ){

    return `
    🌊 <strong>Neelum Valley Group Tour</strong>
    <br><br>
    ...
    ⭐ Included Places:
    Deosai, Shangrila, Katpana Cold Desert, Upper Kachura Lake, Neelum Valley Fort and Manthokha Waterfall
    <br><br>

    🏜 Special Feature:
    Families, honeymoon couples and adventure lovers with lake + mountain + cold desert views
    <br><br>

    💸 Starting Budget:
    Rs. 14,000 per person from Neelum Valley local, higher from other cities
    `;
  }
...
// Places click: show selected place on map
function bindNeelumMapCards() {
  document.querySelectorAll(".place-card").forEach(card => {
    card.addEventListener("click", function () {
      const place = this.dataset.place;
      if (!place) return;
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", bindNeelumMapCards);
} else {
  bindNeelumMapCards();
}
