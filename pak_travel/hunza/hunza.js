...
let seats = Number(localStorage.getItem("hunzaGroupTourSeats")) || 18;
...
const hotels = {
  standardHotel: {
    name: "Standard Hunza Hotel",
    adjustment: 11000,
    label: "Comfortable Karimabad stay"
  },
  premiumHotel: {
    name: "Premium Attabad / Karimabad View Hotel",
    adjustment: 19000,
    label: "Luxury mountain-view rooms"
  },
  budgetGuestHouse: {
    name: "Budget Hunza Guest House",
    adjustment: 7000,
    label: "Low-budget room option"
  },
  familyRoom: {
...
function updateMapAndTopLocation(city) {
  if (!city) {
    routeText.textContent = "Select your starting city to show route to Hunza.";
    routeMap.src = "https://www.google.com/maps?q=Hunza%20Pakistan&output=embed";
    return;
  }

  routeText.textContent = `${city} → Hunza`;
  routeMap.src = `https://www.google.com/maps?q=${encodeURIComponent(city + " to Hunza Pakistan")}&output=embed`;
  ...
  function updatePriceAndRoute() {
    ...
    calculatedRoute.textContent = `${city} to Hunza Group Tour`;
 ...
 function findPreviousBooking(phone) {
  ...
  return bookings
    .filter(b => b.phone === phone && b.tour === "Hunza Group Tour")
    .slice(-1)[0];
}
// function findPreviousBooking(phone) {
//   const bookings = JSON.parse(localStorage.getItem("adminBookings")) || [];
...
// }
  ...
  startLocation.addEventListener("change", function () {
    ...
    if (city === "custom") {
      customLocation.classList.remove("hide");
      priceText.textContent = "Custom price will be confirmed";
      routeText.textContent = "Enter your city to show route to Hunza.";
      routeMap.src = "https://www.google.com/maps?q=Hunza%20Pakistan&output=embed";
      return;
    }
  ...
  bookingForm.addEventListener("submit", function (e) {
    ...
    formData.append("to_destination", "Hunza");
    formData.append("route_text", city + " to Hunza");
    formData.append("hotel_package", hotelChoice.value);
    formData.append("hotel_name", hotel.name);

    // Is Hunza form me vehicle/pickup fields nahi hain, is liye blank save ho raha hai.
      ...
      .then(data => {
        if (data.success) {
          ...
          confirmationMsg.innerHTML =
            ...
            "Route: <strong>" + city + " to Hunza</strong><br>" +
...
function getBotReply(question){
  ...
  /* ========================= */
    ...
    🛣 Route:
    Islamabad → Besham/Naran → Chilas → Hunza
    <br><br>
  ...
  /* ========================= */
    ...
    return `
    🌊 <strong>Hunza Group Tour</strong>
    <br><br>
    ...
    ⭐ Included Places:
    Attabad Lake, Passu Cones, Karimabad, Altit Fort, Baltit Fort and Eagle Nest
    <br><br>
    ...
    💸 Starting Budget:
    Rs. 16,000 per person from Hunza local, higher from other cities
    `;
  }
