...
let seats = Number(localStorage.getItem("swatValleyTourSeats")) || 14;
...
const hotels = {
  standardHotel: {
    ...
    adjustment: 8000,
    label: "Standard comfortable stay"
  },
  premiumHotel: {
    name: "Rock City Resort Swat",
    adjustment: 14000,
    label: "Premium resort package"
  },
  budgetGuestHouse: {
    name: "Budget Swat Guest House",
    adjustment: 5000,
    label: "Economy room option"
  },
  familyRoom: {
    name: "Shelton Rezidor Swat Family Room",
    adjustment: 12000,
    label: "Private family rooms"
  },
  honeymoonSuite: {
    name: "Swat Honeymoon Suite",
    adjustment: 18000,
    label: "Couple-friendly premium room"
  }
};
...
function updateMapAndTopLocation(city) {
  if (!city) {
    routeText.textContent = "Select your starting city to show route to Swat.";
    routeMap.src = "https://www.google.com/maps?q=Swat%20Pakistan&output=embed";
    return;
  }

  routeText.textContent = `${city} → Swat`;
  routeMap.src = `https://www.google.com/maps?q=${encodeURIComponent(city + " to Swat Valley Pakistan")}&output=embed`;
  ...
  function updatePriceAndRoute() {
    ...
    calculatedRoute.textContent = `${city} to Swat Valley Tour`;
 ...
 function findPreviousBooking(phone) {
  ...
  return bookings
    .filter(b => b.phone === phone && b.tour === "Swat Valley Tour")
    .slice(-1)[0];
}
// function findPreviousBooking(phone) {
//   const bookings = JSON.parse(localStorage.getItem("adminBookings")) || [];
...
// }
  ...
  startLocation.addEventListener("change", function () {
    ...
    if (city === "custom") {
      customLocation.classList.remove("hide");
      priceText.textContent = "Custom price will be confirmed";
      routeText.textContent = "Enter your city to show route to Swat.";
      routeMap.src = "https://www.google.com/maps?q=Swat%20Pakistan&output=embed";
      return;
    }
  ...
  bookingForm.addEventListener("submit", function (e) {
    ...
    formData.append("to_destination", "Swat");
    formData.append("route_text", city + " to Swat Valley Tour");
      ...
      .then(data => {
        if (data.success) {
          ...
          confirmationMsg.innerHTML =
            ...
            "Route: <strong>" + city + " to Swat Valley Tour</strong><br>" +
...
function getBotReply(question){
  ...
  /* ========================= */
    ...
    return `
    🌊 <strong>Swat Valley Tour</strong>
    <br><br>
    ...
    ⭐ Included Places:
    Deosai, Shangrila, Katpana Cold Desert, Upper Kachura Lake, Swat Fort and Manthokha Waterfall
    <br><br>
    ...
    💸 Starting Budget:
    Rs. 14,000 per person from Swat local, higher from other cities
    `;
  }
...
// Clickable hotels: image/card click updates the map like Skardu
function openHotelOnMap(card) {
  const hotelLocation = card.dataset.hotel;
  ...
  card.classList.add("active-hotel");
  ...
  const mapCard = document.querySelector(".tour-map-card");
  if (mapCard) {
