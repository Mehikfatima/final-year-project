...
let seats = Number(localStorage.getItem("lahoreTourSeats")) || 14;
...
const hotels = {
  pcHotel: {
    ...
    adjustment: 10000,
    label: "VIP luxury room"
  },
  avariHotel: {
    name: "Avari Lahore Hotel",
    adjustment: 8000,
    label: "Luxury city room"
  },
  ramadaHotel: {
    name: "Ramada by Wyndham Gulberg II",
    adjustment: 6000,
    label: "Premium Gulberg room"
  },
  luxusHotel: {
    name: "Luxus Grand Hotel",
    adjustment: 4000,
    label: "Standard comfortable room"
  },
  falettiHotel: {
    name: "Faletti's Hotel Lahore",
    adjustment: 2500,
    label: "Heritage-style room"
  }
};
...
function updateMapAndTopLocation(city) {
  if (!city) {
    routeText.textContent = "Select your starting city to show route to Lahore.";
    routeMap.src = "https://www.google.com/maps?q=Lahore%20Pakistan&output=embed";
    return;
  }

  routeText.textContent = `${city} → Lahore`;
  routeMap.src = `https://www.google.com/maps?q=${encodeURIComponent(city + " to Lahore Pakistan")}&output=embed`;
  ...
  function updatePriceAndRoute() {
    ...
    calculatedRoute.textContent = `${city} to Lahore`;
 ...
 function findPreviousBooking(phone) {
  ...
  return bookings
    .filter(b => b.phone === phone && b.tour === "Lahore Heritage & Food Tour")
    .slice(-1)[0];
}
// function findPreviousBooking(phone) {
//   const bookings = JSON.parse(localStorage.getItem("adminBookings")) || [];
...
// }
  ...
  startLocation.addEventListener("change", function () {
    ...
    if (city === "custom") {
      customLocation.classList.remove("hide");
      priceText.textContent = "Custom price will be confirmed";
      routeText.textContent = "Enter your city to show route to Lahore.";
      routeMap.src = "https://www.google.com/maps?q=Lahore%20Pakistan&output=embed";
      return;
    }
  ...
  bookingForm.addEventListener("submit", function (e) {
    ...
    formData.append("to_destination", "Lahore");
    formData.append("route_text", city + " to Lahore");
      ...
      .then(data => {
        if (data.success) {
          ...
          confirmationMsg.innerHTML =
            ...
            "Route: <strong>" + city + " to Lahore</strong><br>" +
