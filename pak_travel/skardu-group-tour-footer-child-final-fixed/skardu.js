...
let seats = Number(localStorage.getItem("skarduGroupTourSeats")) || 18;
...
const hotels = {
  standardHotel: {
    name: "Standard Skardu Hotel",
    adjustment: 10000,
    label: "Comfortable mountain stay"
  },
  premiumHotel: {
    name: "Premium Shangrila / Lake View Hotel",
    adjustment: 18000,
    label: "Luxury lake-view rooms"
  },
  budgetGuestHouse: {
    name: "Budget Skardu Guest House",
    adjustment: 6000,
...
function updateMapAndTopLocation(city) {
  if (!city) {
    routeText.textContent = "Select your starting city to show route to Skardu.";
    routeMap.src = "https://www.google.com/maps?q=Skardu%20Pakistan&output=embed";
    return;
  }

  routeText.textContent = `${city} → Skardu`;
  routeMap.src = `https://www.google.com/maps?q=${encodeURIComponent(city + " to Skardu Pakistan")}&output=embed`;
  ...
  function updatePriceAndRoute() {
    ...
    calculatedRoute.textContent = `${city} to Skardu Group Tour`;
 ...
 function findPreviousBooking(phone) {
  ...
  return bookings
    .filter(b => b.phone === phone && b.tour === "Skardu Group Tour")
    .slice(-1)[0];
}
// function findPreviousBooking(phone) {
//   const bookings = JSON.parse(localStorage.getItem("adminBookings")) || [];
...
// }
  ...
  startLocation.addEventListener("change", function () {
    ...
    if (city === "custom") {
      customLocation.classList.remove("hide");
      priceText.textContent = "Custom price will be confirmed";
      routeText.textContent = "Enter your city to show route to Skardu.";
      routeMap.src = "https://www.google.com/maps?q=Skardu%20Pakistan&output=embed";
      return;
    }
  ...
  bookingForm.addEventListener("submit", function (e) {
    ...
    formData.append("to_destination", "Skardu");
    formData.append("route_text", city + " to Skardu Group Tour");
      ...
      .then(data => {
        if (data.success) {
          ...
          confirmationMsg.innerHTML =
            ...
            "Route: <strong>" + city + " to Skardu Group Tour</strong><br>" +
...
function getBotReply(question){
  ...
  /* ========================= */
    ...
    return `
    🌊 <strong>Skardu Group Tour</strong>
    <br><br>
    ...
    ⭐ Included Places:
    Deosai, Shangrila, Katpana Cold Desert, Upper Kachura Lake, Skardu Fort and Manthokha Waterfall
    <br><br>
    ...
    💸 Starting Budget:
    Rs. 14,000 per person from Skardu local, higher from other cities
    `;
  }
