document.addEventListener("DOMContentLoaded", function () {
  ...
  const vehiclePriceText = document.getElementById("vehiclePriceText");
...
// 
let seats = Number(localStorage.getItem("multanSeats")) || 20;
...
const priceByCity = {
  ...
  sukkur: 33000,
  rahimyar: 23000,
  "rahim yar khan": 23000
};

const hotels = {
  ramada: { name: "Ramada by Wyndham Multan", adjustment: 12000, label: "Luxury hotel package" },
  avari: { name: "Avari Xpress Multan", adjustment: 9000, label: "Premium hotel package" },
  hotelOne: { name: "Hotel One Multan", adjustment: 7000, label: "Standard hotel package" },
  sChalet: { name: "S Chalet Multan", adjustment: 6000, label: "Comfort stay package" },
  budgetHotel: { name: "Budget Multan Hotel", adjustment: 4000, label: "Economy package" }
};

const vehicles = {
  "Coaster / Hiace": { name: "Coaster / Hiace", adjustment: 0, label: "group tour" },
  "Private Car": { name: "Private Car", adjustment: 6500, label: "petrol + privacy charge" },
  "Private Jeep": { name: "Private Jeep", adjustment: 8000, label: "old city/shrine route charge" },
  "Luxury Van": { name: "Luxury Van", adjustment: 11000, label: "family comfort charge" }
};
  ...
  function getVehicle() {
    return vehicles[vehicleChoice.value] || null;
  }
...
function updateMapAndTopLocation(city) {
  if (!city) {
    routeText.textContent = "Select your starting city to show route to Multan.";
    routeMap.src = "https://www.google.com/maps?q=Multan%20Pakistan&output=embed";
    return;
  }

  routeText.textContent = `${city} → Multan`;
  routeMap.src = `https://www.google.com/maps?q=${encodeURIComponent(city + " to Multan Pakistan")}&output=embed`;
 let matched = false;
  ...
  function updatePriceAndRoute() {
    ...
    if (!city) {
      ...
      vehiclePriceText.textContent = "Rs. 0";
      calculatedPrice.textContent = "Rs. 0";
      selectedVehicleText.textContent = vehicle ? `${vehicle.name} - ${vehicle.label}` : "Please select vehicle";
      selectedPickupText.textContent = pickupPoint.value ? `${pickupPoint.value} ${pickupAddress.value}` : "Please select pickup point";
    ...
    const hotelExtra = hotel ? hotel.adjustment : 0;
    ...
    const hotelTotal = hotelExtra * rooms;
    const vehicleTotal = vehicleExtra;
    const total = baseTotal + hotelTotal + vehicleTotal;

    calculatedRoute.textContent = `${city} to Multan`;
    selectedHotelText.innerHTML = hotel
      ? `${hotel.name} <span class="hotel-price-badge">${hotel.label}</span>`
      : "Please select hotel";

    if (roomCountText) {
      roomCountText.textContent = `${rooms} room${rooms > 1 ? "s" : ""}`;
    }
    ...
    hotelPriceText.textContent = hotel
      ? `${money(hotelTotal)} (${money(hotelExtra)} x ${rooms} room${rooms > 1 ? "s" : ""})`
      : "Rs. 0";

    vehiclePriceText.textContent = vehicle
      ? `${money(vehicleTotal)} (${money(vehicleExtra)} vehicle charge)`
      : "Rs. 0";
...
function findPreviousBooking(phone) {
  ...
  return bookings
    .filter(b => b.phone === phone && b.tour === "Multan Heritage Tour")
    .slice(-1)[0];
}
  function showPreviousOrForm() {
    ...
    if (previous) {
      previousBookingBox.innerHTML = `
        ...
        <p><strong>Vehicle:</strong> ${previous.vehicleName || "Not selected"}</p>
  ...
  "Custom price will be confirmed";
  ...
  customCity
  ? `${customCity} → Multan`
  : "Enter your city to show route to Multan.";

  routeMap.src =
  customCity
  ? `https://www.google.com/maps?q=${encodeURIComponent(customCity + " to Multan Pakistan")}&output=embed`
  : "https://www.google.com/maps?q=Multan%20Pakistan&output=embed";
  ...
  vehicleChoice.addEventListener(
"change",
updatePriceAndRoute
);

pickupPoint.addEventListener(
"change",
updatePriceAndRoute
);
...
updatePriceAndRoute
  ...
  bookingForm.addEventListener("submit", function (e) {
    ...
    const vehicleTotal = vehicle.adjustment;
    const total = baseTotal + hotelTotal + vehicleTotal;
    ...
    formData.append("to_destination", "Multan");
    formData.append("route_text", city + " to Multan");
      ...
      .then(data => {
        if (data.success) {
          ...
          confirmationMsg.innerHTML =
            ...
            "Route: <strong>" + city + " to Multan</strong><br>" +
...
document.querySelectorAll(".place-card").forEach(card => {
  card.addEventListener("click", function () {
    ...
    document.querySelector(".tour-map-card").scrollIntoView({
      behavior: "smooth"
    });
  });
});
...
document.querySelectorAll(".clickable-hotel").forEach(hotel => {
  hotel.addEventListener("click", function () {
    ...
    document.querySelectorAll(".clickable-hotel").forEach(item => {
      item.classList.remove("active-hotel");
    });
