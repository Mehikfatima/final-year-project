...
let seats = Number(localStorage.getItem("gwadarCoastalTourSeats")) || 18;
...
const hotels = {
  standardHotel: {
    name: "Standard Gwadar Hotel",
    adjustment: 5000,
    label: "Comfortable coastal stay"
  },
  premiumHotel: {
    name: "Premium Sea View Hotel",
    adjustment: 8000,
    label: "Luxury sea-view rooms"
  },
  budgetGuestHouse: {
    name: "Budget Gwadar Guest House",
    adjustment: 2500,
    label: "Low-budget option"
  },
  familyRoom: {
    name: "Family Room Sharing",
    adjustment: 6000,
    label: "Private family stay"
  },
  seaView: {
    name: "Sea View Stay",
    adjustment: 4000,
    label: "Scenic coastal stay"
  }
};
...
function updateMapAndTopLocation(city) {
  if (!city) {
    routeText.textContent = "Select your starting city to show route to Gwadar.";
    routeMap.src = "https://www.google.com/maps?q=Gwadar%20Pakistan&output=embed";
    return;
  }

  routeText.textContent = `${city} → Gwadar`;
  routeMap.src = `https://www.google.com/maps?q=${encodeURIComponent(city + " to Gwadar Pakistan")}&output=embed`;
  ...
  function updatePriceAndRoute() {
    ...
    calculatedRoute.textContent = `${city} to Gwadar Coastal Adventure Tour`;
 ...
 function findPreviousBooking(phone) {
  ...
  return bookings
    .filter(b => b.phone === phone && b.tour === "Gwadar Coastal Adventure Tour")
    .slice(-1)[0];
}
// function findPreviousBooking(phone) {
//   const bookings = JSON.parse(localStorage.getItem("adminBookings")) || [];
...
authForm.addEventListener("submit", function (e) {
  ...
  startLocation.addEventListener("change", function () {
    ...
    if (city === "custom") {
      customLocation.classList.remove("hide");
      priceText.textContent = "Custom price will be confirmed";
      routeText.textContent = "Enter your city to show route to Gwadar.";
      routeMap.src = "https://www.google.com/maps?q=Gwadar%20Pakistan&output=embed";
      return;
    }
  ...
  bookingForm.addEventListener("submit", function (e) {
    ...
    formData.append("to_destination", "Gwadar");
    formData.append("route_text", city + " to Gwadar");
    formData.append("hotel_package", hotelChoice.value);
    formData.append("hotel_name", hotel.name);

    // Is Gwadar form me vehicle/pickup fields nahi hain, is liye blank save ho raha hai.
      ...
      .then(data => {
        if (data.success) {
          ...
          confirmationMsg.innerHTML =
            ...
            "Route: <strong>" + city + " to Gwadar</strong><br>" +
...
function getBotReply(question){
  ...
  /* ========================= */
    ...
    return `
    🌊 <strong>Gwadar Coastal Adventure Tour</strong>
    <br><br>
    ...
    ⭐ Included Places:
    Deosai, Shangrila, Katpana Cold Desert, Upper Kachura Lake, Gwadar Fort and Manthokha Waterfall
    <br><br>
    ...
    💸 Starting Budget:
    Rs. 14,000 per person from Gwadar local, higher from other cities
    `;
  }
